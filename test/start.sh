...
while [ $retcode -ne 0 ]; do
    RES=$(curl -s -w '%{http_code}'  -X PUT -v http://localhost:8085/a1-p/policytypes/2 -H Content-Type:application/json --data-binary @sim_hw.json)
    retcode=$?
...
while [ $retcode -ne 0 ]; do
    ...
    RES=$(curl -s -w '%{http_code}' localhost:8081/a1-policy/v2/rics)
    retcode=$?
    if [ $retcode -eq 0 ]; then
        status=${RES:${#RES}-3}
        if [ $status -ne 200 ] & [ "$RES" == *'"policytype_ids":["2"]'* ]; then
            retcode=1
        fi
    fi
    if [ $retcode -ne 0 ]; then
